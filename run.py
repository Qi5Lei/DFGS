...
def run(model,
    ...
    train_stage2(train_loader_stage1=train_loader_stage1,
...
def train_p1(train_loader_stagep, model, criterion, optimizer, scheduler, args_train,
                      logger_train, log_path, epochs=1):
    ...
    for epoch in range(1, epochs + 1):
        ...
        loss_meter.reset()
        scheduler.step()
        for n_iter, (img, vid, _, _, domain, _) in enumerate(train_loader_stagep):
            optimizer.zero_grad()
            img = img.to(device)
            ...
            if (n_iter + 1) % args_train.log_period == 0:
                logger_train.info("STAGE2-Epoch[{}] Iteration[{}/{}] Loss: {:.3f}, Base Lr: {:.2e}"
                                  .format(epoch, (n_iter + 1), len(train_loader_stagep),
...
def train_stage1(train_loader_stage1, model, optimizer, scheduler, args_train, logger_train, log_path, get_domain=False,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i in range(i_ter):
            ...
            with amp.autocast(enabled=True):
                text_features = model(label=target, get_text=True, domain=domain, cam_label=cid,
...
def train_stage2(train_loader_stage1, train_loader_stage2, model, criterion, optimizer, scheduler, testloaders, args_train, logger_train,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, _, _, domain, cid) in enumerate(train_loader_stage2):
            ...
            with amp.autocast(enabled=True):
                ...
                loss = criterion(score, feat, target, logits, image_features)
